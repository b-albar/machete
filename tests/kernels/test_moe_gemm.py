...
class TestMoeGemmForward:
    ...
    def test_small(self, dtype):
        ...
        _moe_gemm_case(32, 4, 2, 64, 32, dtype,
                       tile_m=32, tile_n=32, tile_k=32)

    @pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
    def test_medium(self, dtype):
        """Medium: 128 tokens, 8 experts, K=128, N=64."""
        _moe_gemm_case(128, 8, 2, 128, 64, dtype,
                       tile_m=64, tile_n=32, tile_k=32)

    @pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
    def test_multi_k_block(self, dtype):
        """Multiple K-blocks: K=128, tile_K=32 → 4 K-blocks."""
        _moe_gemm_case(64, 4, 2, 128, 64, dtype,
                       tile_m=64, tile_n=32, tile_k=32)
    ...
    def test_larger(self, dtype):
        """Larger: 256 tokens, 16 experts, K=256, N=128."""
        _moe_gemm_case(256, 16, 2, 256, 128, dtype,
                       tile_m=64, tile_n=32, tile_k=32)
