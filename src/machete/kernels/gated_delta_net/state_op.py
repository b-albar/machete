...
"""Gated Delta Net State Op — chunked state recurrence.
...
Per chunk:
    ...
    5. h *= exp(g_last) (decay)
    6. h += k^T @ v_gated (state update)
...
class GDNStateOp(Op):
    """Gated Delta Net State — chunked state recurrence.
    ...
    Tensors (native [B, S, NH, K/V] layout):
        ...
        v_new:    (B, S, NH, V)       -- output: new values before gating
    ...
    def __init__(self, **config):
        ...
        #   s_state  [BK,BV] swizzled    (h register→smem staging)
        #   s_uv     [BT,BV]             (u load / v_gated staging)
        #   g_buf    [BT] fp32
    ...
    def compute_mma(
        ...
        if warp_idx < Int32(self.num_mma_warps):
            ...
            # Reusable dtype fragment for conversions (shared for h→smem and v_gated)
            ...
            while chunk_idx < Int32(self.NT_val):
                ...
                # -----------------------------------------------------------
                # v_new = u - w@h, v_gated, write v_new
                # -----------------------------------------------------------
                ...
                for ci in cutlass.range_constexpr(cute.size(acc_vp)):
                    ...
                    row = tCcTV[ci][0]
                    col = tCcTV[ci][1]
                    gVN_tile[Int32(row), Int32(col)] = vn_val.to(self.k_dtype)
