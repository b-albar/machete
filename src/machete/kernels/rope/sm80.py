...
class RopeSM80(SingleKernel, FusableKernel):
    ...
    @cute.jit
    def load_forward(self, logical_idx, smem, q_ptr, cos_ptr, sin_ptr, seq_len, n_tokens):
        ...
        m_cos = cute.make_tensor(
            cos_ptr,
            cute.make_layout((seq_len, half_d), stride=(half_d, 1)),
        )
        m_sin = cute.make_tensor(
            sin_ptr,
            cute.make_layout((seq_len, half_d), stride=(half_d, 1)),
        )
    ...
    @cute.jit
    def compute_forward(self, logical_idx, smem, q_ptr, cos_ptr, sin_ptr, seq_len, n_tokens):
        """
        ...
        mq = cute.make_tensor(
            q_ptr,
            cute.make_layout((n_tokens, n_heads, head_dim), stride=(n_heads * head_dim, head_dim, 1)),
        )
        ...
        for work_idx in range(tidx, total_work, num_threads):
            ...
            mq[m, h_idx, i_idx] = r0.to(self.cute_dtype)
            mq[m, h_idx, i_idx + half_d] = r1.to(self.cute_dtype)

    @cute.jit
    def store_forward(self, logical_idx, smem, q_ptr, cos_ptr, sin_ptr, seq_len, n_tokens):
    ...
    @cute.jit
    def load_backward(self, logical_idx, smem, q_ptr, cos_ptr, sin_ptr, seq_len, n_tokens):
    ...
    @cute.jit
    def compute_backward(self, logical_idx, smem, q_ptr, cos_ptr, sin_ptr, seq_len, n_tokens):
    ...
    @cute.jit
    def store_backward(self, logical_idx, smem, q_ptr, cos_ptr, sin_ptr, seq_len, n_tokens):
