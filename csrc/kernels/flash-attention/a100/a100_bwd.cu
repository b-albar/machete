...
__global__ __launch_bounds__((BWD_PREP_NUM_WORKERS)*kittens::WARP_THREADS, 1) void bwd_prep_ker(const __grid_constant__ bwd_prep_globals<HEAD_DIM, QO_SIZE, KV_SIZE> g)
{
...
__global__ __launch_bounds__(BWD_NUM_WORKERS*kittens::WARP_THREADS, 1) void bwd_attend_ker(const __grid_constant__ bwd_globals<HEAD_DIM, QO_SIZE, KV_SIZE, STAGES> g, AttentionVariant& attention_variant)
{
    extern __shared__ int __shm[];
    shared_allocator al((int *)&__shm[0]);
    ...
    attention_variant.load_data(g, workerid, tic, batch, head, q_start, seq_idx);
    ...
    for (auto qo_idx = q_start; qo_idx < qo_blocks; qo_idx++, tic=(tic+1) % stages) {
        ...
        if (qo_idx + 1 < qo_blocks) {
            ...
            attention_variant.load_data(g, workerid, next_tic, batch, head, qo_idx + 1, seq_idx);
            load_async_wait<1>();
        } else {
            load_async_wait();
        }
        ...
        attention_variant.initialize_step(g, workerid, tic, batch, head, qo_idx, seq_idx);
        ...
        // apply the attention variant to the q, k, v tiles
        ...
        // apply the attention variant to the s block
        attention_variant.logits_transform(g, s_block);
